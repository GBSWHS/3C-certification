...
export default function SessionNav () {
  ...
  return (
    <div className="rounded-md self-center ml-5 p-3 flex gap-3 cursor-default">
      <div>
        <b>{data.name}</b> {data.type ? '선생' : '학생'}님
      </div>
      <span className="hover:underline border-l-2 pl-3 cursor-pointer"><Link href="/logout" passHref><span><FontAwesomeIcon icon={faSignOutAlt} /> 로그아웃</span></Link></span>
    </div>
  )
}
