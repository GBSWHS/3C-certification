...
export default function NoticeEdit () {
  ...
  const { data, error } = useSWR('/api/notices?id=' + id, fetcher)
    ...
    <div className="w-screen">
      ...
      <div className="inline-block w-full align-top p-10">
        <div className="text-center">
          <h1 className="text-center text-4xl border-b-2 font-bold inline-block cursor-default p-2"><FontAwesomeIcon icon={faPenAlt}/> 공지사항 작성</h1>
        </div>
        ...
        <div className="flex justify-center">
          <form onSubmit={onSubmit} className="mt-5 container">
            <input autoFocus type="text" required className="mb-3 w-full border-2 p-3 focus:border-blue-300" maxLength={30} onChange={(ev) => setTitle(ev.target.value)} placeholder="이곳을 눌러 제목을 입력하세요"/>
            <SunEditor setOptions={sunEditorOptions} setDefaultStyle="font-family: 'Noto Sans CJK KR'; font-weight: 400;" defaultValue={'asdf'} placeholder="이곳을 눌러 작성을 시작하세요" onChange={(v) => setContent(v)} height='400px' lang="ko"/>
